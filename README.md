...
- 输入网页章节 URL 后，插件会先加载当前章节；如果当前章节拆成多个 `下一页`，会自动合并这些分页。
- `下一章` / `上一章` 不会在导入时一次性抓完整本，而是在阅读面板中按需加载，避免某个后续坏链导致当前章节无法打开。
- 当前内置 `Sudugu` 站点适配器和通用 HTML 适配器；后续站点可以通过 `RemoteChapterSourceAdapter` 扩展。
- 如果目标站点把请求重定向到其他域名，插件会明确提示失败，并保留当前已经打开的章节内容。
- 插件不会绕过登录、验证码、反爬或站点访问限制；是否可读取取决于目标页面是否真实返回小说正文 HTML。
