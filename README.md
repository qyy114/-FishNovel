...
2. 找到插件包：`build/distributions/FishNovel-1.0.10.zip`。
3. 打开 IntelliJ IDEA。
4. 进入 `Settings -> Plugins`。
5. 点击插件页右上角齿轮按钮，选择 `Install Plugin from Disk...`。
6. 选择 `FishNovel-1.0.10.zip`。
7. 按提示重启 IDEA。
...
$zipPath = Join-Path (Get-Location) "build\distributions\FishNovel-1.0.10.zip"
...
```text
build/distributions/FishNovel-1.0.10.zip
```
